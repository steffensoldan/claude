...
Sprechakt-Registern geordnet, gestaltet wie die erweiterte Ausgabe v5
(Georgia, Er-Form, „Sigle …“-Zeilen) — zusätzlich mit den Register-
Unterüberschriften aus dem Proof of Concept.

Auswahl: REGISTER_BAND_AUSWAHL.md (98 Inschriften aus dem Vollkorpus, 14 je
Register, 0 Überschneidung mit der erweiterten Ausgabe). Nachdichtungen stehen
...
def title(t):    return para(run(t, '<w:b/><w:sz w:val="52"/><w:szCs w:val="52"/>'), f'<w:pPr><w:jc w:val="center"/></w:pPr>')
...
REGISTERS = [
("I","Er war hier","Präsentativ — Dasein als Akt: der bloße Name und der Stein.",[
 ("JaS 16",["Von Bnḥt."]),
 ("HCH 117",["Von ʾnʿm."]),
 ("HCH 156",["Von S¹ḫr."]),
 ("HCH 160",["Von Gḥs²."]),
...
("II","Er wartet","Expressiv — Ausschau und Sehnsucht, im Stein konserviert.",[
...
("III","Er bittet","Direktiv — das Gebet, das im Stein steht.",[
...
("IV","Er klagt","Memorativ — die Trauer als Denkmal; ein Chor aus vielen Händen.",[
...
("V","Er verflucht","Deklarativ — der Fluch, der nie endet.",[
...
("VI","Er bezeugt","Assertiv — Zeugnis für Unbekannte, die später kommen.",[
...
("VII","Er schweigt","Abwesenheitsakt — Lücken, die sprechen; nichts wird geglättet.",[
...
NOTE = [
 ...
 "Jedes Register ist gezielt aus seinem eigenen Korpus-Material gespeist: die Signaturen für I, die Anrufungen für III, die Fluchformeln für V, die fragmentarischen Steine für VII, die datierten für VI. Die Auswahl ist kuratiert und nicht repräsentativ; jede Nachdichtung trägt die OCIANA-Sigle ihrer Quelle.",
...
def main():
    ...
    body_xml = []
    # Titel
    body_xml += [title("Wer dies liest, lebe lang"),
    ...
    # Vorwort
    body_xml += [PAGEBREAK, head("Vorwort")] + [body(p) for p in VORWORT]
    # Register
    for rom, name, sub, poems in REGISTERS:
        body_xml.append(PAGEBREAK)
        body_xml += [roman(rom), regname(name), subtitle(sub)]
        for sg, lines in poems:
            body_xml += [line(l) for l in lines]
            body_xml.append(sigle(sg))
    # Editorische Notiz + Quellen
    body_xml += [PAGEBREAK, head("Editorische Notiz")] + [body(p) for p in NOTE]
    doc = decl + root_open + "<w:body>" + "".join(body_xml) + sect + "</w:body></w:document>"
    # Paket aus v5 kopieren, document.xml ersetzen
    ...
    n = sum(len(p) for *_, p in [(r[0],r[1],r[2],r[3]) for r in REGISTERS])
    print(f"geschrieben: {OUT}  ({sum(len(r[3]) for r in REGISTERS)} Stücke)")
